...
class PerformanceBenchmark:
            ...
            if plan_file.exists():
                try:
                    ...
                    if plan_data.get("processing_mode") == "file_level":
                        ...
                        metrics["processed_files"] = sum(
                            len(g.get("files", [])) for g in groups
                        )
                        metrics["processing_mode"] = "group_based"

                    metrics["contributors_analyzed"] = len(
                        plan_data.get("contributors", {})
                    )
    ...
    def _analyze_iteration_results(self, iteration_results, config):
        """分析迭代结果"""
        analysis = {"config": config, "iterations": len(iteration_results), "modes": {}}
            ...
            if mode_data:
                ...
                if durations:
                    analysis["modes"][mode] = {
                        ...
                        "success_rate": sum(
                            1 for d in mode_data if d.get("success", False)
                        )
                        / len(mode_data),
    ...
    def _print_scenario_summary(self, scenario, results):
        ...
        for mode, data in results["modes"].items():
            ...
            print(
                f"    📊 耗时范围: {data['min_duration']:.2f}s - {data['max_duration']:.2f}s"
            )
            ...
            for scenario_results in self.benchmark_results.values():
                if mode in scenario_results["modes"]:
                    mode_durations.append(
                        scenario_results["modes"][mode]["avg_duration"]
                    )
    ...
    def _collect_system_info(self):
        ...
        try:
            ...
            return {"platform": "未知", "note": "需要安装 psutil 库获取详细系统信息"}
    ...
    def _print_benchmark_summary(self, report):
        ...
        if summary.get("mode_comparison"):
            ...
            for mode, data in summary["mode_comparison"].items():
                print(
                    f"  {mode}: 平均 {data['avg_duration']:.2f}秒 ({data['scenarios_tested']} 个场景)"
                )
...
def main():
    ...
    parser.add_argument(
        "--scenarios", default="simple,complex,large-scale", help="测试场景，逗号分隔"
    )
