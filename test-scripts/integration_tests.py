...
class IntegrationTester:
    ...
    def _test_basic_functionality(self):
        ...
        success = (
            result.returncode == 0 or result.returncode == 1 and  # 1可能表示需要用户交互
            merge_work_dir.exists() and
            plan_file.exists()
        )
    ...
    def _test_file_level_processing(self):
        ...
        try:
            ...
            # 验证文件级结构
            has_file_level = (
                plan_data.get("processing_mode") == "file_level" and
                "files" in plan_data and
                isinstance(plan_data["files"], list)
            )
    ...
    def _generate_report(self):
        ...
        # 保存报告
        report_file = self.test_base_dir / "logs" / f"integration_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
